...
async def start_handler(client: Client, message: Message):
    ...
    await client.send_message(
        ADMIN_CHAT_ID,
        f"↫︙New User Joined The Bot.\n\n  ↫ ID: ❲ {user_id} ❳\n  ↫ Username: ❲ @{message.from_user.username} ❳\n  ↫ Firstname: ❲ {message.from_user.first_name} ❳\n  ↫ Referred by: @{referrer_username}\n  ↫ Total invites of user: {user_doc.get('invites_sent', 0)}\n\n↫︙Total Users of bot: ❲ {total_users} ❳"
    )
...
async def button_handler(client: Client, callback_query):
    ...
    if user_choice == "face_swap":
        ...
        now = datetime.now()
        cooldown_end = cooldown_times.get(user_id)
        ...
        cooldown_times[user_id] = now + timedelta(hours=1)
